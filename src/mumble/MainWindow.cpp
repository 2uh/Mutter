...
void MainWindow::qmChannel_aboutToShow() {
    ...
    //qmChannel->addAction(qaChannelGetMessage);
...
void MainWindow::on_qaChannelCopyURL_triggered() {
    ...
    if(g.db->isTable(host)){
        ...
        if(messageCount < mess.size())
            i = mess.size() - messageCount;
        for(; i < mess.size(); i++) {
...
//void MainWindow::on_qaChannelGetMessage_triggered() {
//    Channel *c = getContextMenuChannel();
//    Channel *p = ClientUser::get(g.uiSession)->cChannel;
//    QString host, uname, pw, channel;
//    unsigned short port;
...
void MainWindow::updateMenuPermissions() {
    ...
    //qaChannelGetMessage->setEnabled(c);
